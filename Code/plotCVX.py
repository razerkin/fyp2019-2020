...
class Vt(object):
    ...
    def __init__(self, es=None, st=None):
        self.st = st
        self.cvx_param = pd.read_csv('./CSV/Background/PARAM/plot/cvx_param_'+str(es)+'_'+str(st)+'.csv')
    ...
    def vtmincvx(self, t=4, matchlist=None, plotcvx=False, workername=None):
        ...
        axes.set(title="Linear Searching Vtmin", xlim=[0.0, t], ylim=[0, 1], xlabel="Time", ylabel="Vt")
        ...
        for j in range(step):
            ...
            print('time ',str(round(tempTime, 2)),"optimal Value", prob.value,s.value,)
            # print("s[]", s.value)
        ...
        if plotcvx:
            ...
            axes.plot(timePlot,vtPlot,label="$S^{tot}$ = "+str(self.st)+"Mbits, $u_{i}$ = 15Mbits/s, $u_{L}^{max}$ = 8Mbits/s")
            plt.scatter(optimalTime, vtMin, color='red', marker='o')
            plt.text(optimalTime, vtMin,'T = ' + str(round(optimalTime, 2)) + 's, Vt = ' + str(round(vtMin, 4))+'J', ha='center', va='bottom', fontsize=14)
            plt.legend()
            plt.show()
            return (timePlot,vtPlot,optimalTime,vtMin)
...
# cx_5 = Vt(es=3,st=4)
# timePlot_5,vtPlot_5,optimalTime_5,vt_5 = cx_5.vtmincvx(matchlist=np.array((1,2,3,4)), plotcvx=False, workername='FULLARRANGEMENT_5')
# print('-----------------------------------------------------------------------')
cx_7 = Vt(es=3,st=4)
timePlot_7,vtPlot_7,optimalTime_7,vt_7 = cx_7.vtmincvx(matchlist=np.array((1,2,3,4)), plotcvx=False, workername='FULLARRANGEMENT_5')
print('-----------------------------------------------------------------------')
# cx_9 = Vt(es=3,st=9)
# timePlot_9,vtPlot_9,optimalTime_9,vt_9 = cx_9.vtmincvx(matchlist=np.array((1,2,3,4)), plotcvx=False, workername='FULLARRANGEMENT_5')
# cx_4 = Vt(es=4)
# timePlot_4,vtPlot_4,optimalTime_4,vt_4 = cx_4.vtmincvx(matchlist=np.array((1,2,3,4,5)), plotcvx=False, workername='FULLARRANGEMENT_4')
# cx_3 = Vt(es=3)
# timePlot_3,vtPlot_3,optimalTime_3,vt_3 = cx_3.vtmincvx(matchlist=np.array((1,2,3,4)), plotcvx=False, workername='FULLARRANGEMENT_3')
# figa, axesa = plt.subplots(nrows=1, ncols=1)
# axesa.set(title="Linear Searching Vtmin", xlim=[0.0, 4], ylim=[0, 0.5], xlabel="Time", ylabel="Vt")
# axesa.plot(timePlot_5, vtPlot_5, label="$S^{tot}$ = 4Mbits, $u_{i}$ = 15Mbits/s, $u_{L}^{max}$ = 8Mbits/s")
# plt.scatter(optimalTime_5, vt_5, color='red', marker='o')
# plt.text(optimalTime_5, vt_5-0.02, 'T:' + str(round(optimalTime_5, 2)) + 's,Vt:' + str(round(vt_5, 4))+'J', ha='center', va='bottom',
#          fontsize=10)
# axesa.plot(timePlot_7, vtPlot_7, label="$S^{tot}$ = 7Mbits, $u_{i}$ = 15Mbits/s, $u_{L}^{max}$ = 8Mbits/s")
# plt.scatter(optimalTime_7, vt_7, color='red', marker='o')
# plt.text(optimalTime_7, vt_7, 'T:' + str(round(optimalTime_7, 2)) + 's,Vt:' + str(round(vt_7, 4))+'J', ha='center', va='bottom',
#          fontsize=10)
# axesa.plot(timePlot_9, vtPlot_9, label="$S^{tot}$ = 9Mbits, $u_{i}$ = 15Mbits/s, $u_{L}^{max}$ = 8Mbits/s")
# plt.scatter(optimalTime_9, vt_9, color='red', marker='o')
# plt.text(optimalTime_9, vt_9+0.02, 'T:' + str(round(optimalTime_9, 2)) + 's,Vt:' + str(round(vt_9, 4))+'J', ha='center', va='bottom',
#          fontsize=10)
# plt.legend()
# plt.show()
